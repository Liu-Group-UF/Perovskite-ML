...
# this code show to generate atom_init.json file in Dr.Xie paper "https://doi.org/10.1103/PhysRevLett.120.145301". The difference is that instead of using one hot encoding to represent different properties value, here we use its absoublue value to represent its values.
...
def numpy_file_to_json(file):
    ...
    out_file = open("atom_abs_value.json", "w")
